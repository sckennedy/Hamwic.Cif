...
namespace Hamwic.Core.Extension
{
    ...
    {
        private static readonly string[] NormaliseStrings =
        {
            " the ", " and ", "t/a", " ltd"
        };
        ...
        {
            var result = new StringBuilder((s ?? "").ToLowerInvariant());

            foreach (var stringToRemove in NormaliseStrings)
            {
                result.Replace(stringToRemove, "");
            }

            return Regex.Replace(result.ToString(), @"^the\s|[^A-Za-z0-9]", "").ToLowerInvariant();
        }

        public static string ToDoubleMetaphoneString(this string self, int maxLength = 4)
        {
            var metaphoneData = new MetaphoneData(maxLength);
            var current = 0;
            ...
            var isSlavoGermanic = (self.IndexOf('W') > -1) || (self.IndexOf('K') > -1) ||
            ...
            while ((metaphoneData.PrimaryLength < maxLength) || (metaphoneData.SecondaryLength < maxLength))
            {
                ...
                switch (workingString[current])
                {
                    ...
                    case 'C':
                        //various germanic
                        if ((current > 1) && !IsVowel(workingString[current - 2]) &&
                            StringAt(workingString, current - 1, "ACH") && (workingString[current + 2] != 'I') &&
                            ((workingString[current + 2] != 'E') ||
                             StringAt(workingString, current - 2, "BACHER", "MACHER")))
                        {
                            metaphoneData.Add("K");
                            current += 2;
                            break;
                        ...
                        if (StringAt(workingString, current, "CC") && !((current == 1) && (workingString[0] == 'M')))
                        {
                            ...
                            if (StringAt(workingString, current + 2, "I", "E", "H") &&
                                !StringAt(workingString, current + 2, "HU"))
                            {
                        ...
                        else if (StringAt(workingString, current + 1, "C", "K", "Q") &&
                                 !StringAt(workingString, current + 1, "CE", "CI"))
                        {
                            current += 2;
                        }
                        else
                    ...
                    case 'G':
                        if (workingString[current + 1] == 'H')
                        {
                            ...
                            if (current < 3)
                            {
                                ...
                                if (current == 0)
                                {
                                    metaphoneData.Add(workingString[current + 2] == 'I' ? "J" : "K");
                                    current += 2;
                                    break;
                                }
                            }
                            ...
                            if (((current > 1) && StringAt(workingString, current - 2, "B", "H", "D")) //e.g., 'bough'
                                || ((current > 2) && StringAt(workingString, current - 3, "B", "H", "D"))
                                //e.g., 'broughton'
                        ...
                        if (workingString[current + 1] == 'N')
                        {
                            ...
                            //not e.g. 'cagney'
                                if (!StringAt(workingString, current + 2, "EY") && (workingString[current + 1] != 'Y') &&
                                    !isSlavoGermanic)
                                {
                        ...
                        if ((current == 0)
                            ...
                            ((workingString[current + 1] == 'Y') ||
                             StringAt(workingString, current + 1, "ES", "EP", "EB", "EL", "EY", "IB", "IL", "IN", "IE",
                                 "EI", "ER")))
                        {
                        ...
                        if ((StringAt(workingString, current + 1, "ER") || (workingString[current + 1] == 'Y')) &&
                            ...
                            && !StringAt(workingString, current - 1, "E", "I") &&
                            !StringAt(workingString, current - 1, "RGY", "OGY"))
                        {
                            metaphoneData.Add("K", "J");
                            current += 2;
                            break;
                    ...
                    case 'J':
                        ...
                        {
                            if (((current == 0) && (workingString[current + 4] == ' ')) ||
                                StringAt(workingString, 0, "SAN "))
                            {
                                metaphoneData.Add("H");
                            }
                            else
                    ...
                    case 'L':
                        if (workingString[current + 1] == 'L')
                        {
                            ...
                            if (((current == self.Length - 3) &&
                                 StringAt(workingString, current - 1, "ILLO", "ILLA", "ALLE"))
                                ||
                                ((StringAt(workingString, last - 1, "AS", "OS") ||
                                  StringAt(workingString, last, "A", "O")) &&
                    ...
                    case 'S':
                        ...
                        {
                            //germanic
                            metaphoneData.Add(StringAt(workingString, current + 1, "HEIM", "HOEK", "HOLM", "HOLZ")
                                ? "S"
                                : "X");
                            current += 2;
                            break;
                    ...
                    case 'T':
                        ...
                        if (StringAt(workingString, current, "TH") || StringAt(workingString, current, "TTH"))
                        {
                            ...
                            if (StringAt(workingString, current + 2, "OM", "AM") ||
                                StringAt(workingString, 0, "VAN ", "VON ") ||
                    ...
                    case 'W':
                        ...
                        if ((current == 0) &&
                            (IsVowel(workingString[current + 1]) || StringAt(workingString, current, "WH")))
                        {
                    ...
                    case 'X':
                        ...
                        if (
                            !((current == last) &&
                              (StringAt(workingString, current - 3, "IAU", "EAU") ||
                               StringAt(workingString, current - 2, "AU", "OU"))))
                        {
                            metaphoneData.Add("KS");
                        }
        ...
        private static bool StringAt(this string self, int startIndex, params string[] strings)
        {
            ...
            return strings.Any(str => self.IndexOf(str, startIndex, StringComparison.OrdinalIgnoreCase) >= startIndex);
        }
        ...
        private class MetaphoneData
        {
            ...
            public MetaphoneData(int maxLength)
            {
            ...
            public override string ToString()
            {
                ...
                if (ret.Length > _maxLength)
                {
                    ret = ret.Substring(0, _maxLength);
                }
